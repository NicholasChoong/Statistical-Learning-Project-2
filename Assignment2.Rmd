...
The largest number $\hat{\sigma}^2$ of factors we can test is the total
number of variables minus one as the $\hat{\sigma}^2$ is calculated by
dividing the difference between the maximum number of variables and the
k. If the k is equal to the maximum number of variables, then we would
be dividing by zero.
